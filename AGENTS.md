...
This file provides guidance to Codex when working with the EduSport backend.
...
This project uses a multi-agent system for quality enforcement. Agents are in `.Codex/agents/`:
...
### Running Agents
```bash
Codex --agent orchestrator "add a new content type for sponsors"
Codex --agent reviewer          # reviews uncommitted changes
Codex --agent api-contract      # validates schemas against frontend types
```
